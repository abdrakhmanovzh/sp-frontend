...
type GetValidNumberConfig = { loop?: boolean; min?: number; max: number }

export function getValidNumber(
  value: string,
  { loop = false, min = 0, max }: GetValidNumberConfig
) {
...
export function getValidArrowNumber(value: string, { step, min, max }: GetValidArrowNumberConfig) {
  let numericValue = parseInt(value, 10)
  if (!isNaN(numericValue)) {
    numericValue += step
    return getValidNumber(String(numericValue), { loop: true, min, max })
  }
  return '00'
}
